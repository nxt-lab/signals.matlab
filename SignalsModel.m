classdef SignalsModel < handle
    ...
    %
    %   (C) 2017-2018 by Truong X. Nghiem (truong.nghiem@gmail.com)
    ...
    methods
        ...
        function P = getInputIndices(self, signal_name, lags)
            % Returns the index/indices in the input vector (as returned
            % from getIOVector) of a signal (of name signal_name) at
            % optional lag values (0, 1, 2,...).  If lags = [], it's
            % equivalent to all lags.
            %
            % For example, getInputIndices('a') returns the indices of all
            % lagged values of signal 'a' in the input vector;
            % getInputIndices('a', [0 2]) returns only the indices of lags
            % 0 and 2 (a[k] and a[k-2]) in the input vector.
            assert(ischar(signal_name));
            all_lags = nargin < 3 || isempty(lags);
            if ~all_lags
                lags = unique(lags);
            end
            
            cur_idx = 1;
            P = false(1, self.m_totalinputs);
            
            for k = 1:numel(self.m_inputs)
                thelags = self.m_inputs{k}{2};
                numlags = numel(thelags);
                if strcmp(self.m_inputs{k}{1}, signal_name)
                    if all_lags
                        % Mark all lags
                        P(cur_idx:(cur_idx + numlags - 1)) = true;
                    else
                        % Mark lags appearing in LAGS
                        P(find(ismember(thelags, lags)) - 1 + cur_idx) = true;
                    end
                end
                cur_idx = cur_idx + numlags;
            end
            P = find(P);
        end
