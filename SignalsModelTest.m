...
%% Test indices in IO vector
idx = mdl.getInputIndices('b');
assert(isequaln(idx, [2 3 4]));

idx = mdl.getInputIndices('b', 0);
assert(isequaln(idx, 4));

idx = mdl.getInputIndices('b', [0 2]);
assert(isequaln(idx, [2 4]));

idx = mdl.getInputIndices('d');
assert(isempty(idx));
